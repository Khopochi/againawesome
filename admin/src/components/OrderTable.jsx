...
export default function OrderTable({rowss}) {
    ...
    const viewCart = (id,name,phone) => {
        navigate("/orders/single/"+id, {state : {name,phone}})
    }
    ...
    const columns = [
      ...
      },
      {
        ...
        width: 110,
        renderCell: (params)=> (
            <span className='view' onClick={()=>viewCart(params.row._id,params.row.userName,params.row.userPhoneNumber)}>Click to view</span>
        )
      },
      {
        field: 'status',
